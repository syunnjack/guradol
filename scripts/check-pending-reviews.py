...
def main() -> None:
    ...
    lines += [
        ...
        '```',
        '',
        '確認が終わったらこの Issue を閉じる。',
        '',
        # **同じ顔ぶれかどうかを、あとで機械が見分けるための印。**
        # 開いたままの Issue があるとき、この印が本文かコメントに既にあれば
        # 何もしない。無ければコメントを足す。これが無いと、Issue を
        # 閉じ忘れている間に来た口コミが、まるごと知らされないまま溜まる。
        f'<!-- pending: {",".join(sorted(str(row["id"]) for row in rows))} -->',
    ]
